...
<e-panel :title="'New Users'" :subtitle="'For the week'">
  ...
  <b-table class="fa-sort-icons table-rounded mt-3 mb-0"
    ...
    <template slot="#" slot-scope="data">
      <b>{{ (perPage * (currentPage - 1)) + data.index + 1 }}</b>
    </template>
  ...
  <b-pagination class="pagination-separated mt-3 mb-0" size="sm" align="center" :total-rows="items.length" :per-page="perPage" v-model="currentPage" />
</e-panel>
```
```js
...
const fields = [
  { key: '#', label: '#', thClass: 'text-center', tdClass: 'text-center' },
  { key: 'name', thClass: 'max-width', tdClass: 'text-nowrap' },
  { key: 'date', tdClass: 'text-nowrap', sortable: true },
